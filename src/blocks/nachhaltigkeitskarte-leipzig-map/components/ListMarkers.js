...
const ListMarkers = ({ markers: initialMarkes, onChange, themeUrl }) => {
    ...
    return (
        <>
        {isOpen && (
            <EditorMarker title={__( 'Edit marker', 'nachhaltigkeitskarte-leipzig' )} onSave={handleSave} onClose={handleClose} themeUrl={themeUrl} {...currentMarker}/>
        )}
        {markers.map((attrs) => (
            <div className="nachhaltigkeitskarte-leipzig-list-item" key={attrs.id}>
                <div>
                ...
                <div>
                    <div>{attrs.label}</div>
                    <div className="nachhaltigkeitskarte-leipzig-list-item-action">
                        <Button isLink isSecondary onClick={ () => handleEdit(attrs.id)}>{__( 'Edit', 'nachhaltigkeitskarte-leipzig' )}</Button>
                        <Button isLink isDestructive onClick={ () => handleDelete(attrs.id)}>{__( 'Delete', 'nachhaltigkeitskarte-leipzig' )}</Button>
                    </div>
                </div>
            </div>
        ))}
