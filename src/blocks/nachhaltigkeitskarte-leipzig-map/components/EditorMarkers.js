...
const Content = ({ text: initialContent, onChange }) => {
    ...
    return (
        <TextareaControl 
        label={__( 'Content', 'nachhaltigkeitskarte-leipzig' )}
        value={ content }
        onChange={ setContent }
        onBlur={ handleBlur }
        help={__( 'You can use html tags', 'nachhaltigkeitskarte-leipzig' )}
        />
    )
}
    ...
    return (
        <Modal
            ...
            <div style={{width: 680, maxWidth: "100%"}}>
                ...
                <div>
                    <Button disabled={!label} isPrimary onClick={ createMarker }> {__( 'Accept', 'nachhaltigkeitskarte-leipzig' )} </Button>
                </div>
            </div>
        </Modal>
    )
