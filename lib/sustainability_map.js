...
<i class="fa fa-info-circle" aria-hidden="true"></i>&nbsp;<span>${data_single.shortdescription}...</span>
    ...
    */
    ...
    jQuery("#redoButton").click(function () {
        ...
        jQuery("#regions_picker").selectpicker("val", "106");
        ...
        jQuery.when.apply(jQuery, web_addresses.map(function (url) {
            ...
            jQuery.each(data, function (ind, elem) {
                ...
                if (elem.latlng == null && elem.full_address != null) {
                    jQuery.getJSON("https://nominatim.openstreetmap.org/search?format=json&q=" + elem.full_address, function (latlng_data) {
                            ...
                            switch (elem.service_type) {
                                ...
                                case "Filiale":
                                    var marker = new L.marker([elem.latlng[0], elem.latlng[1]], {
                                        ID,
                                        icon: blueIcon
                                    }).bindPopup("<span style=color:#4285F4;>" + elem.service_type + "</span><br><b>" + elem.name + "</b><br><button class=marker_click id=" + ID + "_marker>Kurzinfo</button> | <button class=modal_link id=" + ID + "_link>Detailliert</button>").addTo(markers).on("mouseover", mouseFunction);
                                ...
                                case "Veranstaltung":
                                    var marker = new L.marker([elem.latlng[0], elem.latlng[1]], {
                                        ID,
                                        icon: yellowIcon
                                    }).bindPopup("<span style=color:#ffbb33;>" + elem.service_type + "</span><br><b>" + elem.name + "</b><br><button class=marker_click id=" + ID + "_marker>Kurzinfo</button> | <button class=modal_link id=" + ID + "_link>Detailliert</button>").addTo(markers).on("mouseover", mouseFunction);
                                    oms.addMarker(marker);
                                    break;
                                case "Beratungsangebot":
                                case "Bildungsangebot":
                                    var marker = new L.marker([elem.latlng[0], elem.latlng[1]], {
                                        ID,
                                        icon: redIcon
                                    }).bindPopup("<span style=color:#ff4444;>" + elem.service_type + "</span><br><b>" + elem.name + "</b><br><button class=marker_click id=" + ID + "_marker>Kurzinfo</button> | <button class=modal_link id=" + ID + "_link>Detailliert</button>").addTo(markers).on("mouseover", mouseFunction);
                                    oms.addMarker(marker);
                                    break;
                            }
                        }
