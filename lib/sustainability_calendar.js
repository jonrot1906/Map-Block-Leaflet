...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (jQuery(window).width() < 576) {
        jQuery(".tr.calendar-body .calendar-day .day").css({ 'font-size': "11px;" });
    }
    ...
    let eventsCal = new Calendar({
        ...
        <i class="fa fa-info-circle" aria-hidden="true"></i>&nbsp;<span>${event.shortdescription}...</span>
            ...
            if(events_html) {
              ...
              <div class="event-left card-body">
                <div class="row" style="margin-bottom: 5px;">
                    <span class="card_title text-dark" style="font-size:14px;">Leider gibt es heute keine Veranstaltungen. Viel Spaß beim Ausruhen!</span>
                </div>
              </div>
            </div>`;
            }
    ...
    function getAPIData(web_addresses) {
        jQuery("#all_container").LoadingOverlay("show", {
            image: "",
        ...
        jQuery.when.apply($, web_addresses.map(function (url) {
            ...
            jQuery("#all_container").LoadingOverlay("hide");
            var data_helper = [];
            ...
            jQuery.each(data, function (ind, elem) {
                ...
                if (elem.start_at != null && elem.end_at != null) {
                    ...
                    jQuery("#calendar_container").evoCalendar("addCalendarEvent", [{
                        id: elem.id,
                        name: elem.shortname,
                        date: date_arr,
    ...
    jQuery('#calendar_container').on("selectEvent", function (event, activeEvent) {
